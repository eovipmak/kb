<script lang="ts">
    ...
    function updateUrl(changes: { q?: string; type?: string; category?: string; tags?: string[]; page?: number }) {
        const url = new URL($page.url);
        
        if (changes.q !== undefined) changes.q ? url.searchParams.set('q', changes.q) : url.searchParams.delete('q');
        if (changes.type !== undefined) changes.type ? url.searchParams.set('type', changes.type) : url.searchParams.delete('type');
        if (changes.category !== undefined) changes.category ? url.searchParams.set('category', changes.category) : url.searchParams.delete('category');
        if (changes.tags !== undefined) changes.tags.length ? url.searchParams.set('tags', changes.tags.join(',')) : url.searchParams.delete('tags');
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2">
                    {#each COMMON_TAGS as t}
                        <button 
                            class="px-2.5 py-1 rounded-full text-xs font-medium border transition-colors duration-200 {selectedTags.includes(t) ? 'bg-blue-100 text-blue-800 border-blue-200' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}"
        ...
        <div class="flex-1">
            ...
            {:else if results.length > 0}
                <div class="space-y-6">
                    {#each results as result (result.id || result.slug)}
                        <article class="bg-white p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 border border-slate-200">
                        ...
                        <div class="mt-8 text-left max-w-lg mx-auto">
                            ...
                            <ul class="divide-y divide-slate-200">
                                {#each popularArticles.slice(0, 3) as article}
                                    <li class="py-3">
                                        <a href={`/docs/${article.slug}`} class="group flex items-center justify-between">
                                            <span class="text-sm text-slate-700 group-hover:text-blue-600 transition-colors">{article.title}</span>
            ...
            {:else}
                ...
                <div class="text-center py-12">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Type keywords or select filters to browse the knowledge base.</p>
                            ...
                            <h4 class="text-lg font-semibold text-slate-900 mb-4">Popular Articles</h4>
                             <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                {#each popularArticles as article}
                                    <a href={`/docs/${article.slug}`} class="relative rounded-lg border border-slate-300 bg-white px-6 py-5 shadow-sm flex items-center space-x-3 hover:border-slate-400 focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                        <div class="flex-1 min-w-0">
                                            <span class="absolute inset-0" aria-hidden="true"></span>
                                            <p class="text-sm font-medium text-slate-900">{article.title}</p>
                                            <p class="text-sm text-slate-500 truncate">{article.type} &bull; {new Date(article.createdAt).toLocaleDateString()}</p>
                                        </div>
                                    </a>
                                {/each}
                            </div>
