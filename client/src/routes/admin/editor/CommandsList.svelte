...
<div class="bg-[#1e1e1e] border border-gray-800 rounded-md shadow-2xl overflow-hidden min-w-[280px] p-1 flex flex-col">
	{#if items.length}
		{#each items as item, index}
			...
			>
                {#if item.icon}
                    <div class="flex items-center justify-center w-5 h-5 rounded {index === selectedIndex ? 'text-white' : 'text-gray-400'}">
                ...
                <div class="flex flex-col gap-0.5">
                    ...
                    {#if item.description}
                        <span class="text-xs {index === selectedIndex ? 'text-blue-100' : 'text-gray-500'}">{item.description}</span>
                    {/if}
                </div>
			</button>
		{/each}
