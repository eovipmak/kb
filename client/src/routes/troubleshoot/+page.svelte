...
<div class="min-h-screen bg-gray-50 dark:bg-gray-950 py-12 px-4 sm:px-6 lg:px-8">
	<div class="max-w-4xl mx-auto">
		...
		{:else if flows.length === 0}
			<div
				class="text-center py-12 bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800"
			>
				<svg
				...
				<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
					Check back later for new troubleshooting guides.
				</p>
			</div>
		{:else}
			<div class="grid gap-6 md:grid-cols-2">
				{#each flows as flow}
					<a
						href="/troubleshoot/{flow.id}"
						class="block group relative bg-white dark:bg-gray-900 rounded-2xl shadow-sm hover:shadow-xl border border-gray-200 dark:border-gray-800 transition-all duration-300 p-6"
					>
						<div
							class="absolute inset-0 bg-gradient-to-r from-indigo-500/5 to-purple-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity"
						></div>
						<h3
						...
						<div
							class="mt-4 flex items-center text-indigo-600 dark:text-indigo-400 font-medium text-sm group-hover:translate-x-1 transition-transform"
						>
							Start Guide
