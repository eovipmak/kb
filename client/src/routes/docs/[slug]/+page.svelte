<script lang="ts">
	...
	onMount(async () => {
		...
		document.querySelectorAll('pre').forEach((pre) => {
			...
			button.className = 'copy-btn absolute top-2 right-2 bg-gray-700/80 hover:bg-gray-600 text-white text-xs px-2 py-1 rounded opacity-0 transition-opacity duration-200';
...
<div class="min-h-screen bg-gray-50 pb-12 font-sans">
	<nav class="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm backdrop-blur-md bg-white/90">
	...
	<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in text-gray-800">
		<div class="lg:grid lg:grid-cols-12 lg:gap-8">
			<article class="lg:col-span-8 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
				<div class="p-6 md:p-8">
					<header class="mb-8 border-b border-gray-100 pb-8">
						...
						<h1 class="text-3xl sm:text-4xl font-extrabold text-gray-900 mb-4 leading-tight tracking-tight">{data.article.title}</h1>
					...
					<div class="prose prose-blue prose-lg max-w-none hover:prose-a:text-blue-600 prose-img:rounded-lg prose-headings:scroll-mt-24">
						{@html contentHtml}
					</div>
				</div>
			</article>
			...
			<aside class="hidden lg:block lg:col-span-4 space-y-6">
				{#if toc.length > 0}
				<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto">
					<h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">Table of Contents</h3>
					<nav class="space-y-1">
						{#each toc as item}
							<a href="#{item.id}" 
							   ...
							   style="padding-left: {(item.level - 2) * 1 + 0.5}rem; border-left: 2px solid transparent;">
							   {item.text}
							</a>
						{/each}
					</nav>
				...
				<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
					<h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">Related Articles</h3>
					<ul class="space-y-4">
						{#each data.related as item}
							<li>
								<a href="/docs/{item.slug || 'view?id='+item.id}" class="block group">
									<h4 class="text-sm font-medium text-gray-900 group-hover:text-blue-600 transition-colors leading-snug line-clamp-2">
										{item.title}
									</h4>
									<div class="mt-1 flex items-center space-x-2">
										{#if item.tags && item.tags.length > 0}
											<span class="text-xs text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">{Array.isArray(item.tags) ? item.tags[0] : item.tags}</span>
										{/if}
									</div>
								</a>
							</li>
