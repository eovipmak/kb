...
test.describe('Visual Proof Screenshots', () => {
    ...
    test('capture sequence of user journey', async ({ page, browserName }) => {
        ...
        await page.route('**/api/qa?*', async (route) => {
            const json = {
                data: [
                    { id: '1', title: 'Getting Started', slug: 'getting-started', type: 'FAQ', createdAt: new Date().toISOString() },
                    { id: '2', title: 'Authentication 101', slug: 'auth-101', type: 'GUIDE', createdAt: new Date().toISOString() }
                ],
        ...
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '01_homepage_docs.png'), fullPage: true });
        ...
        await page.route('**/api/qa*', async (route) => {
            ...
            const json = {
                data: [
                    { id: '1', title: 'Getting Started', slug: 'getting-started', status: 'PUBLISHED', type: 'FAQ', author: { email: 'admin@example.com' }, createdAt: new Date().toISOString() },
                    { id: '2', title: 'Draft Article', slug: 'draft-article', status: 'DRAFT', type: 'FAQ', author: { email: 'admin@example.com' }, createdAt: new Date().toISOString() }
                ],
                pagination: { total: 2, page: 1, limit: 10, totalPages: 1 }
            };
            await route.fulfill({ json });
        ...
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '02_login_admin_dashboard.png'), fullPage: true });
        ...
        await page.route('**/api/categories', async (route) => {
            await route.fulfill({ json: [{ id: '1', name: 'General' }, { id: '2', name: 'Billing' }] });
        });
        ...
        await page.locator('.ProseMirror').pressSequentially('First, log in to the system. Then navigate to the docs section.', { delay: 50 });

        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '03_writing_article.png'), fullPage: true });
        ...
        await page.route('**/api/qa/slug/**', async (route) => {
            ...
            const json = {
                ...
                slug: 'how-to-use-kb',
                contentHtml: '<h1>How to Use the Knowledge Base</h1><p>First, log in...</p><img src="https://placehold.co/600x400" />',
                type: 'FAQ',
                status: 'PUBLISHED',
        ...
        // Check availability
        try {
            await expect(page.getByRole('heading', { level: 1 })).toContainText('How to Use the Knowledge Base', { timeout: 5000 });
        } catch (e) {
            console.log('Heading not found or incorrect. Page content:', await page.content());
        }

        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '07_preview_article.png'), fullPage: true });
        ...
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '08_edit_article.png'), fullPage: true });
        ...
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '05_audit_history.png'), fullPage: true });
